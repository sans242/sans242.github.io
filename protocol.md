...
<style>
  ...
  /* LINK INPUTS */
  .add-link-form { display: flex; flex-direction: column; gap: 8px; margin-bottom: 15px; }
  .link-input { 
    background: #2d2d2d; border: 1px solid #444; color: #fff; 
    padding: 8px; border-radius: 4px; font-size: 0.9rem;
  }
  .add-btn { 
    background: #008AFF; color: white; border: none; padding: 8px; 
  ...
  .ref-url { font-size: 0.75rem; color: #008AFF; text-decoration: none; white-space: nowrap; overflow: hidden; text-overflow: ellipsis; max-width: 180px; }
  .delete-btn { color: #666; background: none; border: none; cursor: pointer; font-size: 1.2rem; margin-left: 5px; }
  ...
  @media (max-width: 900px) {
    ...
    .protocol-wrapper { flex-direction: column; }
    .sidebar-column { flex: none; width: 100%; }
    .refs-box { position: static; }
  }
  ...
  .status-btn[data-status="fail"] { background: #4a1c1c; border-color: #9b2c2c; color: #f56565; }
  ...
  .relock-link:hover { color: #888; text-decoration: underline; }
...
<div class="protocol-wrapper" style="display: none;">
    ...
    <div class="logs-grid">
      ...
      <div class="log-card">
        <div class="card-header">
          ...
          <button class="status-btn" id="btn-prob" onclick="toggleStatus('probability')">SET STATUS</button>
        </div>
        <textarea class="log-input" id="txt-prob" placeholder="Log progress..."></textarea>
      </div>
      ...
      <div class="log-card">
        <div class="card-header">
          ...
          <button class="status-btn" id="btn-dsa" onclick="toggleStatus('dsa')">SET STATUS</button>
        </div>
        <textarea class="log-input" id="txt-dsa" placeholder="Log progress..."></textarea>
      </div>
      ...
      <div class="log-card">
        <div class="card-header">
          ...
          <button class="status-btn" id="btn-project" onclick="toggleStatus('project')">SET STATUS</button>
        </div>
        <textarea class="log-input" id="txt-project" placeholder="Log progress..."></textarea>
      </div>
    </div>
...
<script>
  ...
  const wrapper = document.querySelector('.protocol-wrapper');
  ...
  const dateText = document.getElementById('date-display-text');
  
  const els = {
    probability: { txt: document.getElementById('txt-prob'), btn: document.getElementById('btn-prob') },
    dsa:         { txt: document.getElementById('txt-dsa'),  btn: document.getElementById('btn-dsa') },
    project:     { txt: document.getElementById('txt-project'), btn: document.getElementById('btn-project') }
  };
  ...
  // --- DATA LOGIC ---
  function getStorageKey(date) { return `protocol_log_${date}`; }
  ...
  function saveLogData() {
    ...
    ['probability', 'dsa', 'project'].forEach(cat => {
      data[cat] = {
        text: els[cat].txt.value,
        status: els[cat].btn.getAttribute('data-status') || 'neutral'
      };
    });
    ...
    links.forEach((link, i) => {
      ...
      li.innerHTML = `
        <div class="ref-link-wrapper">
          ...
          <a href="${link.url}" target="_blank" class="ref-url" title="${link.url}">${niceUrl}</a>
        </div>
        <button class="delete-btn" onclick="deleteLink(${i})">×</button>
      `;
      list.appendChild(li);
    });
  }
  ...
  function addNewLink() {
    const titleIn = document.getElementById('new-link-title');
